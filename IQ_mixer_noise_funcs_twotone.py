...
def init_daq(meas_dict, fs=None):
    '''
    Initialize the DAQ NI 9775
    3 channels (over 4) are being used (chan0: I, chan1: Q, chan2: modulation signal, chan3: I wave form)
    ...
    task.ai_channels.add_ai_voltage_chan(chan3_str)
    ...
    # if fs is not None:
    #     assert (20e6/fs)/4 is integer
    
    if meas_dict['mode'] == 'freq_sweep':
        if fs is None:
            meas_dict['Npts_period'] = int(10e5) # Num of points per sweep (arbitrary, change if necessary)
            ...
            meas_dict['DAQ_freq'] = int(fs)
            meas_dict['Npts_period'] = int(meas_dict['DAQ_freq']/meas_dict['rate'])
            meas_dict['DAQ_Npts'] = int(2 * meas_dict['Npts_period']) 
        ...
        if fs is None:
            ...
            meas_dict['DAQ_freq'] = int(fs)
            meas_dict['Npts_period'] = int(meas_dict['DAQ_freq']/meas_dict['rate'])
            meas_dict['DAQ_Npts'] = int(21 * meas_dict['Npts_period']) 
        ...
        time_noise = 100 #seconds
        meas_dict['DAQ_freq'] = int((20/16)*1e6) # Hz, DAQ acquisition frequency #07/31/2024 maximum=(20/12)*1e6 in high_res mode and maximum=2e6 in high_speed mode
        meas_dict['DAQ_Npts'] = int(meas_dict['DAQ_freq']*100) #100 is measurement time, to capture noise over 10 s #04/28/2024 change the value to 1e6(1s) from 10e6 #06/04/2024 change the value to 30e6(30s) from 10e6
...
def get_cryo_LNA_gain(cryo_chan, freq):
    ...
    freq_range_chan2 = (10e6, 2000e6) # LNC0.6_2A 
...
def init_measurement(main_dict):
    ...
    if main_dict['IQ_mixer'] == 'SigaTek QD54A10':
        ...
        main_dict['LO_pwr'] = 13 # dBm: for Sigatek QD54A10 mixer, the input LO power must be between 10 and 13 dBm
        ...
        main_dict['RFpwr_req@mixer'] = 10 # dBm: Desired power level for mixer RF input #-10
        main_dict['board'] = 'LF'
    elif main_dict['IQ_mixer'] == 'TSC AD0460B':
    ...
    if main_dict['SweepMode'] == '2_Res':
        for i_res in range(main_dict['Nres']):
            if abs(main_dict['freq_center'][i_res]-main_dict['second_res'][i_res])>main_dict['F_sampling']/2:
                print('The two resonances are too far apart. Nyquist limit not able to be achieved')
                return
...
def set_SR560(mode, SweepMode='DC'):
    SR560_dict = {'Model':'SR560', 'gain_mode':'low_noise'}   
    
    assert SweepMode=='DC' or SweepMode=='AC' or SweepMode=='2_Res', "Wrong SweepMode set, you can only chose 'DC' or 'AC' "
    
    # if mode == 'freq_sweep':
    #     if SweepMode == 'DC':
    #         LP_freq=3e4
    #     elif SweepMode == 'AC' or SweepMode == '2_Res':
    #         LP_freq = 100e4 # Hz make it a floating quantity from the carrier tones: need at least 50kHz around the tones, two tones need to be f0 +2Delta f appart
    #         # LP_freq = 1e6 # Hz make it a floating quantity from the carrier tones: need at least 50kHz around the tones, two tones need to be f0 +2Delta f appart
    #     gain = 20
    #     if SweepMode == 'DC':
    #         SR560.set_DC_filters(LP=LP_freq) #comment for two tone
    #     elif SweepMode == 'AC' or SweepMode == '2_Res':
    #         SR560.set_AC_LPfilter(LP=LP_freq) #comment for two tone
    #     time.sleep(1)
    #     SR560.set_gain(gain)
    #     SR560_dict['coupling'] = SweepMode
...
def init_synthesizer_SG382(main_dict, meas_dict):
    ...
    if meas_dict['mode'] == 'freq_sweep':
        ...
        SG382.write('FREQ %d'%(meas_dict['freq_center']-meas_dict['span']))
        print('frequency set to %.6f MHz'%(float(SG382.query('FREQ?'))*1e-6))
        time.sleep(0.1)
        SG382.write('TYPE 1') # modulation type (0: AM, 1: FM, 2: PM, 3: Sweep, 4: Pulse, 5: Blank)
        print('Frequency modulation set')
    ...
    if meas_dict['mode'] == 'calib':
        with h5py.File(h5_filepath, 'a') as hf:  
            ...
            for key in ['IQ_mixer', 'dev_name', 'cryo_chan', 'dir_path', 
                        ...
                        'time_hhmmss', 'SG382_pwr', 'Modulation_Tones']:#main_dict.keys():
...
def save_data_noise(main_dict, meas_dict, SR560_dict, data_dict, h5_filepath, i):
    if meas_dict['mode'] == 'noise':
        with h5py.File(h5_filepath, 'a') as hf:  
            ...
            m_onres_dataset = gr_noise.create_dataset('WaveFormG' + str(i), data=data_dict['WaveFormG'])
...
def take_freq_sweep(main_dict, meas_dict, SG382, task):
    ...
    data_dict = {'freq':[], 'I_data':[], 'Q_data':[], 'Mod_data':[], 'WaveForm_data': []}
    ...
    for i_band in range(meas_dict['N_bands']):
        ...
        WaveForm_data = daq_data[3,i_start:i_stop]
        ...
        data_dict['WaveForm_data'] = np.concatenate([data_dict['WaveForm_data'], WaveForm_data])
...
def freq_sweep_main(main_dict, meas_dict, CarrierTones, F_sampling, BandWidth, Filter_order = 10, SweepMode='DC', plot=True):

    assert SweepMode == 'DC' or SweepMode == 'AC' or SweepMode == '2_Res', "Wrong entry for the SweepMode, set it to either 'DC' or 'AC' or '2_Res'."  
    #critical, in high res mode, only possible values of Fs is 20e6/(4*N) Hz (where N is integer). Also need to put assert so that BW + f1 is below fs/2
    mode = 'freq_sweep'
    ...
    task, meas_dict = init_daq(meas_dict, F_sampling)
    fs = meas_dict['DAQ_freq']
    two_res_loop_break = True

    # Npts = meas_dict['DAQ_Npts']
    # S_time = Npts/fs
    # timedata = np.arange(0, S_time, 1/fs)
    SR560_dict = set_SR560(mode, SweepMode)
    ...
    for i_res in range(main_dict['Nres']):
        ...
        if SweepMode == 'AC':
            ...
            I_data, Q_data =  \
                Process2tones.numerical_demodulation(data_dict[i_res]['I_data'], data_dict[i_res]['Q_data'], fs, CarrierTones, BandWidth,\
                                                    Filter_order = Filter_order, WaveForm_data = data_dict[i_res]['WaveForm_data'],\
                                                        PhaseCorrection = False, Throw=False, plots = True, Plots_dir = sweep_dir)
            I_data=I_data[0]
            Q_data=Q_data[0]
            # throw = meas_dict['Npts_period'] - I_data.shape[0]
            Freq_offset = CarrierTones[0]
            ...
            I_data = data_dict[i_res]['I_data']
            Q_data = data_dict[i_res]['Q_data']
            
            Npts = data_dict[i_res]['I_data'].shape[0]
            S_time = (Npts)/fs
            timedata = np.linspace(0, S_time, Npts, endpoint=False)
            fI, AmpI, AngI = Process2tones.FFT(data_dict[i_res]['I_data'],S_time)
            fQ, AmpQ, AngQ = Process2tones.FFT(data_dict[i_res]['Q_data'],S_time)
            peaks, _ = sig.find_peaks(np.abs(AmpI+1j*AmpQ), prominence=0.001, distance=200e3*S_time)
            print(peaks)
            
            fig, axs = plt.subplots(nrows=2, ncols=2, gridspec_kw=dict(hspace=0.5, wspace=0.3), figsize=(16, 9))
            axs[0][0].plot(fI, AmpI, color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
            axs[0][0].set_xlabel('frequency [Hz]', fontsize = 12)
            axs[0][0].set_ylabel('amplitude (I FFT)', fontsize = 10)
            
            axs[1][0].plot(fQ, AmpQ, color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
            axs[1][0].set_xlabel('frequency [Hz]', fontsize = 12)
            axs[1][0].set_ylabel('amplitude (Q FFT)', fontsize = 10)
            
            axs[0][1].plot(timedata, data_dict[i_res]['I_data'], color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
            axs[0][1].set_xlabel('Time [s]', fontsize = 12)
            axs[0][1].set_ylabel('I data', fontsize = 10)
            
            axs[1][1].plot(timedata, data_dict[i_res]['Q_data'], color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
            axs[1][1].set_xlabel('Time [s]', fontsize = 12)
            axs[1][1].set_ylabel('Q data', fontsize = 10)
            
            sweep_dir = main_dict['plot_dir'] + 'freq_sweep/'
            os.makedirs(sweep_dir, exist_ok=True)
            filename = sweep_dir + 'ControlPlot'
            print('Saving the ControlPlot')
            fig.savefig(filename + '.pdf', dpi=100)
        
        S21_dB = 10*np.log10(I_data**2 + Q_data**2)
        phase = np.arctan2(Q_data, I_data) * 180 / np.pi
        # phase = np.arctan2(data_dict[i_res]['Q_data'], data_dict[i_res]['I_data']) * 180 / np.pi  # Phase of S21 in degrees
        
        peaks, _ = sig.find_peaks(-1*S21_dB, prominence=1, distance=1e3) # distance is the min distance between peaks #change prominence for peak high detection, default is 2
        fres = data_dict[i_res]['freq'][peaks]
        i_fres_target = np.argmin(np.abs(fres - main_dict['freq_center'][i_res]))
        ...
        data_dict[i_res]['fres_target'] = data_dict[i_res]['fres'][i_fres_target]
        if SweepMode == '2_Res':
            i_off_fres_target = np.argmin(np.abs(fres - main_dict['second_res'][i_res]))
            data_dict[i_res]['second_res_target'] = data_dict[i_res]['fres'][i_off_fres_target]
    ...
    task.close()  
    time.sleep(2)
    main_dict['freq_center'] = np.array([b['fres_target'] for b in data_dict])

    # data_dict = find_freq_sweep_reso(main_dict, data_dict)
    if SweepMode =='2_Res':
        for ii in data_dict:
            assert ii['fres_target'] != ii['second_res_target']
            main_dict['second_res'] = np.array([a['second_res_target'] for a in data_dict])
    ...
    if plot == True:
        figs = []
        for i_res in range(main_dict['Nres']):
            freq = data_dict[i_res]['freq'] + Freq_offset
            freq_onres = data_dict[i_res]['fres'] + Freq_offset
            freq_onres_target = data_dict[i_res]['fres_target'] + Freq_offset
            freq_offres = freq_onres_target + main_dict['df_onoff_res'] + Freq_offset
            ...
            #     tone1_I_cos = np.cos(fI[peaks[0]]*2*np.pi*timedata+AngI[peaks[0]])
            #     tone1_Q_cos = np.cos(fQ[peaks[0]]*2*np.pi*timedata+AngQ[peaks[0]])
            #     I_data = data_dict[i_res]['I_data']*(tone1_I_cos*2)
            #     Q_data = data_dict[i_res]['Q_data']*(tone1_Q_cos*2)
            #     I_data=Process2tones.butter_lowpass_filter(I_data, LP_cutoff, fs, 10)
            #     Q_data=Process2tones.butter_lowpass_filter(Q_data, LP_cutoff, fs, 10)
            # elif SweepMode == 'DC':
            #     I_data = data_dict[i_res]['I_data']
            #     Q_data = data_dict[i_res]['Q_data']
            
            # S21_dB = 10*np.log10(I_data**2 + Q_data**2)
            # phase = np.arctan2(Q_data, I_data) * 180 / np.pi  # Phase of S21 in degrees
            
            # Create a 2-row, 1-column subplot
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 12), dpi=100)
            plt.subplots_adjust(bottom=0.12, top=0.9, right=0.96, left=0.10, hspace=0.35)
            
            # Top plot: S21 in dB
            ax1.set_xlabel('Frequency LO + tone [MHz]', fontsize=16)
            ax1.set_ylabel(r'$|S_{21}|$ [dBV]', fontsize=16)
            ax1.tick_params(color='k', labelsize=14)
            ax1.plot(freq / 1e6, S21_dB, 'C0', linewidth=2, label='raw')
            
            freq_onres_str = ''
            for peak_freq in freq_onres / 1e6:
                freq_onres_str += '%.3f MHz\n' % peak_freq
                
            # ax1.plot(freq[i_onres] / 1e6, S21_dB[i_onres], 'ok', linewidth=2, label=freq_onres_str)
            ax1.plot(freq[i_onres_target] / 1e6, S21_dB[i_onres_target], '*C3', markersize=12, label='On res')
            ax1.plot(freq[i_offres] / 1e6, S21_dB[i_offres], '*C2', markersize=12, label='Off res')
            
            if SweepMode == 'DC':
                data_dict_calib[i_res]['I_data'], data_dict_calib[i_res]['Q_data'] = main_calib_IQ_DCdata_d(
                    I_data, Q_data, data_dict[i_res]['freq'], IQ_calibfile_path=IQ_calibfileDC_path)
            elif SweepMode == 'AC' or SweepMode == '2_Res':
                data_dict_calib[i_res]['I_data'], data_dict_calib[i_res]['Q_data'] = main_calib_IQ_DCdata_d(
                    I_data, Q_data, data_dict[i_res]['freq'], IQ_calibfile_path=IQ_calibfileAC_path)
        
            S21_dB_calib = 10 * np.log10(data_dict_calib[i_res]['I_data']**2 + data_dict_calib[i_res]['Q_data']**2)
            phase_calib = np.arctan2(data_dict_calib[i_res]['Q_data'], data_dict_calib[i_res]['I_data']) * 180 / np.pi  # Phase of S21 in degrees
            
            ax1.plot(freq / 1e6, S21_dB_calib, 'r', linestyle=':', linewidth=2, label='add IQ Mixer calib')
            
            ax1.legend(fontsize=14)
            ax1.grid()
            
            # Bottom plot: Phase of S21
            ax2.set_xlabel('Frequency LO + tone [MHz]', fontsize=16)
            ax2.set_ylabel(r'Phase [Degrees]', fontsize=16)
            ax2.tick_params(color='k', labelsize=14)
            ax2.plot(freq / 1e6, phase, 'C0', linewidth=2, label='Phase')
            ax2.plot(freq / 1e6, phase_calib, 'r', linestyle=':', linewidth=2, label='Phase calib')
            ax2.legend()
            
            ax2.grid()
            
            figtitle = '%s:  %dmK  %ddBm  on res:%.2fMHz  off res:%.2fMHz' % (
                main_dict['dev_name'], int(main_dict['FPtemp_ini'] * 1000), 
                int(main_dict['req_pwr@device'][0]), freq[i_onres_target] / 1e6, freq[i_offres] / 1e6)
...
def fit_res(data_dict, CarrierTone, mode='lmfit'):
    freq = data_dict['freq'] + CarrierTone[0]#W
    I = data_dict['I_data']
    Q = data_dict['Q_data']
    N = len(freq)
    df = freq[1] - freq[0]
    ...
    # resObj.load_params(scr.cmplxIQ_params, fit_quadratic_phase = True, hardware = 'mixer') # hardware = VNA or mixer
    ...
    #print("hallo")
    #f_start_arg = fres_arg - df_arg+int(N*0.05)
    #f_stop_arg = fres_arg + df_arg-int(N*0.05)
    #for key in ['I', 'Q', 'freq']:
    #     dataDict[key] = dataDict[key][f_start_arg:f_stop_arg]
    #resObj = scr.makeResFromData(dataDict)                                
    #resObj.load_params(scr.cmplxIQ_params, fit_quadratic_phase = True, hardware = 'mixer')
    # # resObj.load_params(scr.cmplxIQ_params, fit_quadratic_phase = True, hardware = 'mixer')
    ...
    resfit_dict['result_I'] = resObj.resultI
    resfit_dict['result_Q'] = resObj.resultQ
    ...
    Span_Start = resfit_dict['fres'] - span/2
    Span_End = fres + span/2
    
    i_fres = np.argmin(np.abs(fres - resfit_dict['freq_data']))
    i_start = np.argmin(np.abs(Span_Start - resfit_dict['freq_data']))
    i_end = np.argmin(np.abs(Span_End - resfit_dict['freq_data']))
    

    resfit_dict['calibrated_I'] = resObj.resultI
    resfit_dict['calibrated_Q'] = resObj.resultQ
    
    resfit_dict['i_fres'] = i_fres
    resfit_dict['i_start'] = i_start
    resfit_dict['i_end'] = i_end

    
    dI = np.gradient(resfit_dict['calibrated_I'])
    dQ = np.gradient(resfit_dict['calibrated_Q'])
    
    angle = np.arctan2(dI, dQ)
    
    resfit_dict['angle'] = angle[i_fres]
...
def FM_main(main_dict, meas_dict, CarrierTones, F_sampling, BandWidth, Filter_order = 10, SweepMode='DC'):#, i_res):
    ...
    SR560_dict = set_SR560(meas_dict['mode'], SweepMode)
    time.sleep(2)
    SG382 = init_synthesizer_SG382(main_dict, meas_dict)
    SG382.write('ENBR 1')
    task, meas_dict = init_daq(meas_dict, fs = F_sampling)
    time.sleep(0.5)
    fs = meas_dict['DAQ_freq']
    LP_cutoff = 50000
    ...
    freq_start = meas_dict['freq_center'] - meas_dict['span']/2 #+ CarrierTones[0]
    freq_stop = meas_dict['freq_center'] + meas_dict['span']/2 #+ CarrierTones[0]
    
    # print(f"Frequency range have been shifted by + {CarrierTones[0]} Hz")
    
    data_dict['freq'] = np.linspace(freq_start, freq_stop, Npts_period_meas)
    print("measurement range is ", data_dict['freq'])
    ...
    data_dict['WaveForm_data'] = daq_data[3,:]
    
    if SweepMode == 'AC':
        
        FitRes_dir = main_dict['plot_dir'] + 'fit_res/'
        
        I_data, Q_data = Process2tones.numerical_demodulation(
            daq_data[0,:], daq_data[1,:], fs, CarrierTones, BandWidth, Filter_order = Filter_order, WaveForm_data = data_dict['WaveForm_data'],\
                PhaseCorrection = False, plots = True, Plots_dir = FitRes_dir, Special_Sweep = True,  Noise_debug = True)
        I_data=I_data[0][i_start:i_stop]
        Q_data=Q_data[0][i_start:i_stop]
        # I_data = Process2tones.butter_lowpass_filter(daq_data[0,:], 5e3, fs)
        # Q_data = Process2tones.butter_lowpass_filter(daq_data[1,:], 5e3, fs)
        # I_data=I_data[i_start:i_stop]
        # Q_data=Q_data[i_start:i_stop]
        ...
        I_data, Q_data = Process2tones.numerical_demodulation(
            daq_data[0,:], daq_data[1,:], fs, CarrierTones, BandWidth, Filter_order = Filter_order, WaveForm_data = data_dict['WaveForm_data'],\
                PhaseCorrection = False, plots = True, Plots_dir = FitRes_dir, Special_Sweep = True)
        #here you need to take the demodulated data of tone 2
        I_data=I_data[1][i_start:i_stop]
        Q_data=Q_data[1][i_start:i_stop]
        # I_data = Process2tones.butter_lowpass_filter(daq_data[0,:], 5e3, fs)
        # Q_data = Process2tones.butter_lowpass_filter(daq_data[1,:], 5e3, fs)
        # I_data=I_data[i_start:i_stop]
        # Q_data=Q_data[i_start:i_stop]
        ...
        I_data = Process2tones.butter_lowpass_filter(daq_data[0,:], 10e3, fs)
        Q_data = Process2tones.butter_lowpass_filter(daq_data[1,:], 10e3, fs)
        I_data=I_data[i_start:i_stop]
        Q_data=Q_data[i_start:i_stop]
        
        Npts = daq_data[0,i_start:i_stop].shape[0]
        S_time = (Npts)/fs
        timedata = np.linspace(0, S_time, Npts, endpoint=False)
        fI, AmpI, AngI = Process2tones.FFT(daq_data[0,i_start:i_stop],S_time)
        fQ, AmpQ, AngQ = Process2tones.FFT(daq_data[1,i_start:i_stop],S_time)
        peaks, _ = sig.find_peaks(np.abs(AmpI+1j*AmpQ), prominence=0.001, distance=200e3*S_time)
        print(peaks)
        ...
        axs[0][1].plot(timedata, daq_data[0,i_start:i_stop], color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
        axs[0][1].set_xlabel('Time [s]', fontsize = 12)
        axs[0][1].set_ylabel('I data', fontsize = 10)
        
        axs[1][1].plot(timedata, daq_data[1,i_start:i_stop], color='C%d'%0, markersize=5, alpha=1, rasterized=True, label='before demodulation')
        axs[1][1].set_xlabel('Time [s]', fontsize = 12)
        axs[1][1].set_ylabel('Q data', fontsize = 10)
        
        FitRes_dir = main_dict['plot_dir'] + 'fit_res/'
        os.makedirs(FitRes_dir, exist_ok=True)
        filename = FitRes_dir + 'ControlPlot'
        print('Saving the ControlPlot')
        fig.savefig(filename + '.pdf', dpi=100)
    ...
    if SweepMode == 'DC':
        data_dict['I_data_calib'], data_dict['Q_data_calib'] = main_calib_IQ_DCdata_d(
            data_dict['I_data'], data_dict['Q_data'], data_dict['freq'], IQ_calibfile_path=IQ_calibfileDC_path)
    elif SweepMode == 'AC' or SweepMode == '2_Res':
        data_dict['I_data_calib'], data_dict['Q_data_calib'] = main_calib_IQ_ACdata_d(
            data_dict['I_data'], data_dict['Q_data'], data_dict['freq']+ CarrierTones[0], IQ_calibfile_path=IQ_calibfileAC_path)
...
def plot_fit_reso(resfit_dict, main_dict, meas_dict):
    ...
    fres = resfit_dict['fres']
    resObj = resfit_dict['resObj']
    Span_Start = fres - span/2
    Span_End = fres + span/2
    i_fres = np.argmin(np.abs(fres - resfit_dict['freq_data']))
    i_start = np.argmin(np.abs(Span_Start - resfit_dict['freq_data']))
    i_end = np.argmin(np.abs(Span_End - resfit_dict['freq_data']))
    S21_data_norm = (resfit_dict['I_data'] + 1j*resfit_dict['Q_data'])
    S21_data_norm_mag_dB = 20 * np.log10(np.abs(S21_data_norm))
    S21_fit_norm_mag_dB = 20 * np.log10(np.abs(resObj.resultMag))
    
    center = resObj.resultI[i_fres] + 1j*resObj.resultQ[i_fres]
    print(center)
    
    angle = resfit_dict['angle']
    
    dI = 0.01*np.sin(angle)
    dQ = 0.01*np.cos(angle)
    
    
    
    
    axs[0].plot(resObj.I,resObj.Q, '.C0', markersize=3, label='data add IQ Mixer calib')
    axs[0].plot(resObj.resultI, resObj.resultQ, 'k', linewidth=2, label='fit add IQ Mixer calib')
    axs[0].plot(resObj.resultI[i_fres], resObj.resultQ[i_fres], '*C1', markersize=10, label='fres add IQ Mixer calib')
    axs[0].plot(resObj.resultI[i_start],resObj.resultQ[i_start], 'xk', markersize=10, label='Rotation Span')
    axs[0].plot(resObj.resultI[i_end],resObj.resultQ[i_end], 'xk', markersize=10)
    axs[0].plot([np.real(center)-dI, np.real(center)+dI], [np.imag(center)-dQ, np.imag(center)+dQ],color='r', marker='o', label = 'fit_calib_angle')

    axs[1].plot(resfit_dict['freq_data']/1e6, S21_data_norm_mag_dB, '.C0', markersize=3, label='data add IQ Mixer calib')
    axs[1].plot(resfit_dict['freq_data']/1e6, S21_fit_norm_mag_dB, 'k', linewidth=2, label='fit add IQ Mixer calib')
    axs[1].plot(fres/1e6, S21_fit_norm_mag_dB[i_fres], '*C1', markersize=10, label='fres add IQ Mixer calib')
    axs[1].plot(Span_Start/1e6, S21_fit_norm_mag_dB[i_start], 'xk', markersize=10, label='Rotation Span')
    axs[1].plot(Span_End/1e6, S21_fit_norm_mag_dB[i_end], 'xk', markersize=10)
    
    angle = np.angle(S21_fit_norm_mag_dB[i_end]-S21_fit_norm_mag_dB[i_start])
    print(f'estimated angle = {angle} rad')
    ...
    # i_start = np.argmin(np.abs(Span_Start - resfit_dict['freq_data_raw']))
    # i_end = np.argmin(np.abs(Span_End - resfit_dict['freq_data_raw']))
    ...
    # axs[0].plot(np.real(S21_norm_fit[i_start]), np.imag(S21_norm_fit[i_start]), 'xk', markersize=10)
    # axs[0].plot(np.real(S21_norm_fit[i_end]), np.imag(S21_norm_fit[i_end]), 'xk', markersize=10)
    # axs[1].plot(resfit_dict['freq_data_raw']/1e6, S21_data_norm_mag_dB, '.g', markersize=3, label='data_raw')
    # axs[1].plot(resfit_dict['freq_data_raw']/1e6, S21_fit_norm_mag_dB, 'r', linewidth=2, label='fit_raw')
    # axs[1].plot(fres/1e6, S21_fit_norm_mag_dB[i_fres], '*b', markersize=10, label='fres_raw')
    # axs[1].plot(Span_Start/1e6, S21_fit_norm_mag_dB[i_start], 'xk', markersize=10)
    # axs[1].plot(Span_End/1e6, S21_fit_norm_mag_dB[i_end], 'xk', markersize=10)
    ...
    # fig2, axs2 = plt.subplots(1, 2, figsize=(17, 10), dpi=100)
    # # plt.subplots_adjust(bottom=0.12, top=0.9, right=0.96, left=0.10, wspace=0.28)
    # # title_str = fig.text(0.4, 0.94, '', fontsize=16)
    # axs2[0].set_xlabel('I [V]', fontsize=16)
    # axs2[0].set_ylabel('Q [V]', fontsize=16)
    # axs2[0].tick_params(color='k', labelsize =14)
    # axs2[0].tick_params(color='k', labelsize =14)
    # axs2[0].grid()
    # axs2[1].set_xlabel('I [V]', fontsize=16)
    # axs2[1].set_ylabel('Q [V]', fontsize=16)
    # axs2[1].tick_params(color='k', labelsize =14)
    # axs2[1].tick_params(color='k', labelsize =14)
    # axs2[1].grid()
    ...
    # S21_data_norm_mag_dB = 20 * np.log10(np.abs(S21_data_norm))
    # S21_fit_norm_mag_dB = 20 * np.log10(np.abs(S21_norm_fit))
    # axs2[0].plot(np.real(S21_data_norm), np.imag(S21_data_norm), '.C0', markersize=3, label='data add IQ Mixer calib')
    # axs2[0].plot(np.real(S21_norm_fit), np.imag(S21_norm_fit), 'k', linewidth=2, label='fit add IQ Mixer calib')
    # axs2[0].plot(np.real(S21_norm_fit[i_fres]), np.imag(S21_norm_fit[i_fres]), '*C1', markersize=10, label='fres add IQ Mixer calib')
    ...
    # S21_data_norm_mag_dB = 20 * np.log10(np.abs(S21_data_norm))
    # S21_fit_norm_mag_dB = 20 * np.log10(np.abs(S21_norm_fit))
    # axs2[1].plot(np.real(S21_data_norm), np.imag(S21_data_norm), '.g', markersize=3, label='data_raw')
    # axs2[1].plot(np.real(S21_norm_fit), np.imag(S21_norm_fit), 'r', linewidth=2, label='fit_raw')
    # axs2[1].plot(np.real(S21_norm_fit[i_fres]), np.imag(S21_norm_fit[i_fres]), '*b', markersize=10, label='fres_raw')
...
def take_calib(main_dict, meas_dict, resfit_dict, i_res, F_sampling, CarrierTones = 0, SweepMode = 'DC'):
    
    print(f'Displacing fres by the Carrier tone: {CarrierTones} Hz')
    ...
    # if SweepMode == 'DC':
    #     CarrierTones = 0
    meas_dict['freq_center'] = fres - CarrierTones[0]
    print(f"The resonance Frequency is {fres*1e-6} MHz. The center frequency for the LO is {meas_dict['freq_center']*1e-6} MHz")
    meas_dict['span'] = fres / Qr / 10
    # meas_dict['span'] = 10e3*2
    print(f"The span is {meas_dict['span']} Hz")
    meas_dict['f_cutoff'] = fres / 2 / Qr
    meas_dict['freq_start'] = meas_dict['freq_center'] - meas_dict['span'] / 2
    meas_dict['freq_stop'] = meas_dict['freq_center'] + meas_dict['span'] / 2
    
    # in the current version of the code with two on tone measurement, main_dict['freq_center'] is not an array and have no length
    # meas_dict['freq_start'] = main_dict['freq_center'][i_res] - meas_dict['span'] / 2
    # meas_dict['freq_stop'] = main_dict['freq_center'][i_res] + meas_dict['span'] / 2
    ...
    # main_dict = init_measurement(main_dict)
    # meas_dict = set_atten_and_pwr(main_dict, meas_dict)
    SR560_dict = set_SR560('FM', SweepMode)
    time.sleep(5)
    SG382 = init_synthesizer_SG382(main_dict, meas_dict)
    SG382.write('ENBR 1')
    task, meas_dict = init_daq(meas_dict, fs = F_sampling)
    ...
    # meas_dict['freq_center'] = fres + CarrierTones
    # meas_dict['span'] = fres / Qr / 10
    # meas_dict['f_cutoff'] = fres / 2 / Qr
    # meas_dict['freq_start'] = main_dict['freq_center'][i_res] - meas_dict['span'] / 2
    # meas_dict['freq_stop'] = main_dict['freq_center'][i_res] + meas_dict['span'] / 2
    
    data_dict = {'I_data':daq_data[0], 'Q_data':daq_data[1], 'Mod_data':daq_data[2], 'WaveForm_data':daq_data[3]}
    task.close()
    SG382.write('ENBR 0')
    return main_dict, meas_dict, SR560_dict, data_dict


def calc_calib(data_dict, meas_dict, main_dict, CarrierTones, BandWidth, Filter_order = 10, SweepMode='DC'):
    
    WF_data = data_dict['WaveForm_data']
    Mod_data = data_dict['Mod_data']
    fs = meas_dict['DAQ_freq']
    Npts = Mod_data.shape[0]
    Npts_reduction = int(Npts*0.05)
    Mod_data = Mod_data[Npts_reduction:-Npts_reduction]

    
    if SweepMode == 'AC' or SweepMode == '2_Res':
        
        CalibRot_dir = main_dict['plot_dir'] + 'calib_rotation/'
        
        I_data, Q_data = Process2tones.numerical_demodulation(
            data_dict['I_data'], data_dict['Q_data'], fs, CarrierTones, BandWidth,\
                Filter_order = Filter_order, WaveForm_data = WF_data,  PhaseCorrection = False, plots = True, Plots_dir = CalibRot_dir, Special_Sweep = False,  Noise_debug = True)#, Plots_dir =  CalibRot_dir)
        
        I_data=np.array((I_data[0][Npts_reduction:-Npts_reduction]))
        Q_data=np.array((Q_data[0][Npts_reduction:-Npts_reduction]))
        WF_data = WF_data[Npts_reduction:-Npts_reduction]
    ...
    calib_dict['I_data']=I_data[Mod_i]
    calib_dict['Q_data']=Q_data[Mod_i]
    calib_dict['Mod_data']=Mod_data[Mod_i]
    calib_dict['WaveForm_data']=WF_data[Mod_i]
...
# def calc_calib(data_dict, meas_dict, main_dict, CarrierTones, BandWidth, Filter_order=10, SweepMode='DC'):
#     """
#     Calculate calibration data by analyzing I and Q data over a square modulation sweep.

#     Parameters:
#     - data_dict: Dictionary containing waveform and modulation data.
#     - meas_dict: Dictionary containing measurement metadata.
#     - main_dict: Dictionary containing calibration paths and plot settings.
#     - CarrierTones: List of carrier tones for demodulation.
#     - BandWidth: Bandwidth for filtering.
#     - Filter_order: Order of the filter.
#     - SweepMode: 'AC' for dynamic sweep, 'DC' for static data.

#     Returns:
#     - calib_dict: Dictionary containing calibration results.
#     """
#     WF_data = data_dict['WaveForm_data']
#     Mod_data = data_dict['Mod_data']
#     fs = meas_dict['DAQ_freq']
...
#     # Fit sine wave to modulation data to identify half-periods
#     def func_sin(i, a, b):
#         return np.sin((i - a) * b)

#     a_ini, b_ini = 0, 2 * np.pi / meas_dict['Npts_period']
#     popt, _ = curve_fit(func_sin, np.arange(len(Mod_data)), Mod_data, p0=[a_ini, b_ini])
#     mod_sin = func_sin(np.arange(len(Mod_data)), popt[0], popt[1])

#     # Identify regions for high and low half-periods of modulation
#     Mod_low_i = np.where(mod_sin < -0.8)[0]
#     Mod_high_i = np.where(mod_sin > 0.8)[0]

#     # Split data into contiguous blocks for each half-period
#     def split_into_blocks(indices):
#         """
#         Split contiguous indices into separate blocks for each half-period.
#         """
#         blocks = []
#         start = indices[0]
#         for i in range(1, len(indices)):
#             if indices[i] != indices[i - 1] + 1:
#                 blocks.append(np.arange(start, indices[i - 1] + 1))
#                 start = indices[i]
#         blocks.append(np.arange(start, indices[-1] + 1))
#         return blocks

#     low_blocks = split_into_blocks(Mod_low_i)
#     high_blocks = split_into_blocks(Mod_high_i)
...
#     #     calib_dict['WaveForm_data_raw']=WF_data[Mod_i]
...
#     def demodulate_block(block, freq_value, directory = None):
#         """
#         Demodulate a specific block of data.
#         """
#         block_I = data_dict['I_data'][block]
#         block_Q = data_dict['Q_data'][block]
#         block_WF = WF_data[block]

#         if SweepMode == 'AC':
#             block_I, block_Q = Process2tones.numerical_demodulation(
#                 block_I, block_Q, fs, CarrierTones, BandWidth,
#                 Filter_order=Filter_order, WaveForm_data=block_WF,
#                 PhaseCorrection=True, plots=False)#, Plots_dir = directory)
#             block_I = block_I[0]
#             block_Q = block_Q[0]

#         return block_I, block_Q, freq_value * np.ones(len(block))
...
#     Ilow=[]
#     Qlow=[]
#     # Process low half-periods
#     for block in low_blocks:
#         Directory = CalibRot_dir + '/low/'+f'{block}'
#         I_low, Q_low, freq_low = demodulate_block(block, meas_dict['freq_start'], Directory)
#         Ilow.append(np.mean(I_low))
#         Qlow.append(np.mean(Q_low))
#         calib_dict['I_low'].extend(I_low)
#         calib_dict['Q_low'].extend(Q_low)
#         calib_dict['freq_low'].extend(freq_low)
#     plt.plot(Ilow,Qlow,'.r')
    
#     Ihigh=[]
#     Qhigh=[]
#     # Process high half-periods
#     for block in high_blocks:
#         Directory = CalibRot_dir + '/High/'+f'{block}'
#         I_high, Q_high, freq_high = demodulate_block(block, meas_dict['freq_stop'], Directory)
#         Ihigh.append(np.mean(I_high))
#         Qhigh.append(np.mean(Q_high))
#         calib_dict['I_high'].extend(I_high)
#         calib_dict['Q_high'].extend(Q_high)
#         calib_dict['freq_high'].extend(freq_high)
#     plt.plot(Ihigh,Qhigh,'.b')

#     # Calculate IQ calibration parameters
#     # IQ_low_mean = np.mean(calib_dict['I_low']) + 1j * np.mean(calib_dict['Q_low'])
#     # IQ_high_mean = np.mean(calib_dict['I_high']) + 1j * np.mean(calib_dict['Q_high'])
#     IQ_low_mean = np.median(Ilow) + 1j * np.median(Qlow)
#     IQ_high_mean = np.median(Ihigh) + 1j * np.median(Qhigh)
#     plt.plot(IQ_low_mean.real,IQ_low_mean.imag,'xr')
#     plt.plot(IQ_high_mean.real,IQ_high_mean.imag,'xb')
...
#     print(f"Calibration angle: {angle * 180 / np.pi} degrees")
#     dIQ_mag = np.sqrt((IQ_high_mean.real - IQ_low_mean.real)**2 +
#                       (IQ_high_mean.imag - IQ_low_mean.imag)**2)
#     dIQ_vs_df = dIQ_mag / meas_dict['span']

#     # Update calibration results
#     calib_dict.update({
#         'IQ_mean': [IQ_low_mean, IQ_high_mean],
#         'angle': angle,
#         'dIQ_vs_df': dIQ_vs_df,
#     })
...
def calc_calib_calib(data_dict,resfit_dict, meas_dict):

    I_data = data_dict['I_data']/np.real(resfit_dict['baseline'])
    Q_data = data_dict['Q_data']/np.real(resfit_dict['baseline'])
    Mod_data = data_dict['Mod_data']
...
def noise_main(main_dict, meas_dict, SG382, task):
    ...
    noise_dict = {'I_onres':daq_onres[0], 'Q_onres':daq_onres[1],'WaveFormG':daq_onres[2]}
...
def plot_calib_rotation(calib_dict, main_dict, meas_dict, resfit_dict, fig, old_axs):
    
    # Save the old plot's data or handles if needed
    old_lines = old_axs[0].lines  # Get the line objects from the old plot
    
    fig, axs = plt.subplots(1, 1, figsize=(17, 10), dpi=100)
    plt.subplots_adjust(bottom=0.12, top=0.9, right=0.96, left=0.10, wspace=0.28)
    # Add elements from the old plot to the new plot
    for line in old_lines:
        x_data, y_data = line.get_data()
        axs.plot(x_data, y_data, color=line.get_color(), label = line.get_label())
        
    axs.set_xlabel('I [V]', fontsize=16)
    axs.set_ylabel('Q [V]', fontsize=16)
    axs.tick_params(color='k', labelsize =14)
    axs.tick_params(color='k', labelsize =14)
    axs.axis('equal')
    axs.grid()
    IQ_mean_new_calib = np.array(calib_dict['IQ_mean']) * np.exp(-1j*calib_dict['angle'])
    
    axs.plot(calib_dict['I_data'], calib_dict['Q_data'],  '.', color='y', label='calib data')
    axs.plot(calib_dict['IQ_mean'][0].real, calib_dict['IQ_mean'][0].imag, '*r', markersize=10)
    axs.plot(calib_dict['IQ_mean'][1].real, calib_dict['IQ_mean'][1].imag, '*c', markersize=10)
    axs.plot([calib_dict['IQ_mean'][0].real, calib_dict['IQ_mean'][1].real], [calib_dict['IQ_mean'][0].imag, calib_dict['IQ_mean'][1].imag], 'g', linewidth=2)
    axs.plot(IQ_mean_new_calib[0].real, IQ_mean_new_calib[0].imag, '*r', markersize=10)
    axs.plot(IQ_mean_new_calib[1].real, IQ_mean_new_calib[1].imag, '*c', markersize=10)
    axs.plot([IQ_mean_new_calib[0].real, IQ_mean_new_calib[1].real], [IQ_mean_new_calib[0].imag, IQ_mean_new_calib[1].imag], 'g', linewidth=2)
    ...
    time.sleep(5)
    # plt.close('all')
def plot_noise(calib_dict, noise_dict, timestream_factor = 0.01, decimation_factor = 0.01, I_demodulated = None, Q_demodulated = None):
    fig, axs = plt.subplots(1, 2, figsize=(17, 10), dpi=100)
    plt.subplots_adjust(bottom=0.12, top=0.9, right=0.96, left=0.10, wspace=0.28)
    axs[1].set_xlabel('I [V]', fontsize=16)
    axs[1].set_ylabel('Q [V]', fontsize=16)
    axs[1].tick_params(color='k', labelsize =14)
    axs[1].tick_params(color='k', labelsize =14)
    axs[1].axis('equal')
    axs[1].grid()
    ...
    n = int(timestream_factor*decimation_factor*len(I_demodulated))
    print(n)
    
    
    indices_to_keep = random.sample(range(int(len(I_demodulated)*timestream_factor)), n)
    indices_to_keep.sort()
    
    print(indices_to_keep)
    
    #if I_demodulated ==None and Q_demodulated == None:
    #    decimated_I = noise_dict['I_onres_raw'][indices_to_keep]
    #    decimated_Q = noise_dict['Q_onres_raw'][indices_to_keep]
    #else:
    decimated_I_1 = I_demodulated[0][indices_to_keep]
    decimated_Q_1 = Q_demodulated[0][indices_to_keep]
    
    decimated_I_2 = I_demodulated[1][indices_to_keep]
    decimated_Q_2 = Q_demodulated[1][indices_to_keep]
    time_stream = np.linspace(0, 100*timestream_factor, n)    
    
    #axs[1].plot(calib_dict['I_data'], calib_dict['Q_data'],  '.', color='y', label='calib data')
    #axs[1].plot(calib_dict['IQ_mean'][0].real, calib_dict['IQ_mean'][0].imag, '*r', markersize=10)
    #axs[1].plot(calib_dict['IQ_mean'][1].real, calib_dict['IQ_mean'][1].imag, '*c', markersize=10)
    #axs[1].plot([calib_dict['IQ_mean'][0].real, calib_dict['IQ_mean'][1].real], [calib_dict['IQ_mean'][0].imag, calib_dict['IQ_mean'][1].imag], 'g', linewidth=2)
    axs[0].plot(time_stream, decimated_I_1, color = 'blue', label = 'I timestream')
    axs[0].plot(time_stream, decimated_Q_1, color = 'red', label = 'Q timestream')
    axs[1].plot(decimated_I_1, decimated_Q_1, color = 'm', label = 'Q timestream')
...
def plot_Sdff(Sdff_dict, main_dict, meas_dict, ylims=[1e-20, 1e-14]):
    ...
    axs[0].loglog(Sdff_dict['f'], np.abs(Sdff_dict['PSD_onres_freq_avg']), linewidth=2, color='C0', linestyle=':', label = 'calib onres: freq')
    axs[0].loglog(Sdff_dict['f'], np.abs(Sdff_dict['PSD_onres_diss_avg']), linewidth=2, color='C1', linestyle=':', label = 'calib onres: diss')
    axs[0].loglog([meas_dict['f_cutoff'], meas_dict['f_cutoff']], ylims, linewidth=2, color='k', linestyle=':', label = 'resonance cutoff')
    ...
    # axs[1].loglog(Sdff_dict['f'], np.abs(Sdff_dict['PSD_offres_freq_avg']), linewidth=2, color='C0', linestyle=':', label = 'calib offres: freq')
    # axs[1].loglog(Sdff_dict['f'], np.abs(Sdff_dict['PSD_offres_diss_avg']), linewidth=2, color='C1', linestyle=':', label = 'calib offres: diss')
    # axs[1].loglog([meas_dict['f_cutoff'], meas_dict['f_cutoff']], ylims, linewidth=2, color='k', linestyle=':', label = 'resonance cutoff')
